...
data class ColorHSL(
    ...
    override fun toColorInt(): ColorInt {
        return hslToRgb(hue, saturation, lightness).toColorInt()
    }

}


fun ColorInt.toHSL(): ColorHSL {
    val rgb = this.toRgb()

    val hsl = rgbToHsl(rgb.red, rgb.green, rgb.blue)

    return ColorHSL(hsl[0], hsl[1], hsl[2])
}
...
fun ColorHSL.asRgb(): ColorRGB = toColorInt().toRgb()

fun ColorHSL.asArgb(): ColorARGB = toColorInt().toArgb()
 ...
 */
fun ColorHSL.lighten(value: Float): ColorHSL {
    ...
    return ColorHSL(hue, saturation, mutableLightness)
}

/**
 * @param value amount to lighten in the range 0...100
 */
fun ColorHSL.lighten(value: Int): ColorHSL {
    var mutableLightness = this.lightness
 ...
 */
fun ColorHSL.darken(value: Float): ColorHSL {
    var mutableLightness = this.lightness
    mutableLightness -= value
    mutableLightness = 0f.coerceAtLeast(mutableLightness.coerceAtMost(1f))
    return ColorHSL(hue, saturation, mutableLightness)
 ...
 */
fun ColorHSL.darken(value: Int): ColorHSL {
    var mutableLightness = this.lightness
    mutableLightness -= value / 100f
    mutableLightness = 0f.coerceAtLeast(mutableLightness.coerceAtMost(1f))
    return ColorHSL(hue, saturation, mutableLightness)
 ...
 */
fun ColorHSL.shades(count: Int = 10): List<ColorHSL> {
 ...
 */
fun ColorHSL.tints(count: Int = 10): List<ColorHSL> {
    require(count > 0) { "count must be > 0" }
 ...
 */
fun ColorHSL.complimentary(): ColorHSL {
 ...
 */
fun ColorHSL.triadic(): Pair<ColorHSL, ColorHSL> {
 ...
 */
fun ColorHSL.tetradic(): Triple<ColorHSL, ColorHSL, ColorHSL> {
 ...
 */
fun ColorHSL.analogous(): Pair<ColorHSL, ColorHSL> {
